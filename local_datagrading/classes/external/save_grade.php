...
/**
 * Web service: save a grade for a single database entry.
 */
class save_grade extends external_api {
     ...
     */
    public static function execute(int $cmid, int $recordid, int $fieldid, float $grade, string $feedback = ''): array {
        global $DB, $USER;
        ...
        // Validate the field belongs to this activity.
        $field = $DB->get_record('data_fields', ['id' => $fieldid, 'dataid' => $cm->instance, 'type' => 'gradeentry'], '*', MUST_EXIST);

        // Validate the record belongs to this activity.
        $record = $DB->get_record('data_records', ['id' => $recordid, 'dataid' => $cm->instance], 'id, userid', MUST_EXIST);
        ...
        // Persist the grade value in data_content.
        ...
        // Save grade metadata and sync gradebook.
        \local_datagrading\grade_manager::save($cmid, $recordid, $grade, $feedback, (int) $USER->id);

        // Fire event (observer does second gradebook sync pass; primary sync already done in grade_manager::save).
