 ...
 * Database activity.
 *
 * @module     local_datagrading/inline_grader
 ...
 *
 * Called by local_datagrading_before_footer() via $PAGE->requires->js_call_amd.
 *
...
export const init = (cmid, contextid) => { // eslint-disable-line no-unused-vars
    getStrings([
        {key: 'savinggrade',      component: 'local_datagrading'},
        {key: 'graded',           component: 'local_datagrading'},
        {key: 'errorsavegrade',   component: 'local_datagrading'},
        {key: 'gradingprogress',  component: 'local_datagrading'},
...
const wireReleaseControls = (cmid) => {
    document.querySelectorAll('[data-gradeentry-release]').forEach(checkbox => {
        checkbox.addEventListener('change', () => {
            ...
            Ajax.call([{
                methodname: 'local_datagrading_release_grades',
...
const wireReleaseAllButton = (cmid) => {
    const btn = document.getElementById('local-datagrading-release-all');
    if (!btn) {
        return;
    }
    btn.addEventListener('click', () => {
        ...
        Ajax.call([{
            ...
            args: {cmid, recordids: []},
                ...
                const progressText = document.getElementById('local-datagrading-progress-text');
                if (progressText) {
...
const triggerSave = (cmid, gradeInput, fieldid, gradeValue, feedback, savingStr, gradedStr, errorStr) => {
    ...
    Ajax.call([{
        methodname: 'local_datagrading_save_grade',
        args: {
            cmid,
            recordid,
...
const updateProgressBar = (graded, total) => {
    const bar = document.getElementById('local-datagrading-progress');
    if (!bar) {
        return;
    }
    ...
    const text = document.getElementById('local-datagrading-progress-text');
    if (text) {
